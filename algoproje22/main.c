    ...
    for(int i=0;i<N;i++){
        ...
        for(int j=0;j<S;j++){
               if(ogrenci_cevaplari[j][i]==cevap_anahtari[j])
                HBN[i]+=soru_degeri; //her dogru cevapta ogrencinin basari notu soru degeri kadar artırılır.
...
void tablo(double ortalama,double T_skoru[ ],int N,double h_notu[ ])
{ //tablodaki verilere gore once sinif ortalamasiyla sinif duzeyi ardından sinif duzeyine gore de harf notu hesaplanilan fonksiyon cagrildi.
